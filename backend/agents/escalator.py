...
Be 2-3 sentences max. Return ONLY the message text, no JSON, no formatting.
"""
...
async def _generate_message(task: dict) -> str:
    prompt = (
        ...
        f"Status: {task['status']}\nRisk score: {task.get('risk_score', 0):.2f}"
    )
    try:
...
async def run_escalator(
    stalled_tasks: list[dict],
    workspace_id: int = 1
) -> tuple[list[Alert], AuditEntry]:
    audit = AuditEntry(
        agent_name="EscalatorAgent", action="generate_escalations",
        input_summary=f"Processing {len(stalled_tasks)} stalled tasks",
        output_summary="", status="success"
    )
    try:
        all_tasks = await get_all_tasks(workspace_id)
        high_risk = [
        ...
        alerts = []
        for task in stalled_tasks + high_risk:
            message = await _generate_message(task)
            alert   = Alert(
                task_id=task["id"], task_title=task["title"],
                alert_type="stalled" if task["status"] == "stalled" else "sla_breach",
                message=message, severity=_severity(task.get("risk_score", 0.5))
            )
            alert.id = await insert_alert(alert, workspace_id)
            alerts.append(alert)
